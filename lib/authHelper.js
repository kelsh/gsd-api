...
const DB = require(path.resolve( './databaseConnection.js') );
...
class AuthHelper{
	...
	async auth(req){
		...
		if(req.headers.sessiontoken){
			...
			console.log('get valid user', validUser)
			if(validUser){
				return validUser;
			}
		...
		console.log('authhelper getting user by username and pass',  req.headers.username, req.password)
		...
		console.log('authhelper just tried to lookup user by username and pass', validUser)
		if(!validUser){
			console.log('not valid user')
			//you're not a valid user, maybe you should sign up?
			return false;
			console.log('not valid user2')
		}
		console.log('were passing by the return fasel')
		...
		//set new token
		await redisClient.set(newToken, validUser.userid);
	...
	async validateToken(sessionToken){
		...
		let valid = await getRedisAsync(sessionToken).then(async (reply) =>{
		    ...
		    if(reply){
		    	...
		    	console.log('returning found user from validate token', foundUser)
		    	return foundUser;
